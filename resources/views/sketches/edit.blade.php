...
<body>
    <div id="app">
        ...
        <div class="d-flex container-fluid px-4 mt-3 ">
            <form action="{{route('sketch.delete', $sketch->id)}}" method="POST" enctype="multipart/form-data" class="d-flex justify-content-between align-items-center w-100 bgTopbar" id="sketchF"  @keydown.enter="preventFormSubmit">
                @csrf
                @method('put')
                ...
                <!-- TITLE -->
                <div class="d-flex">
                    <input type="text" name="name" id="name" value="{{$sketch->name}}" class="text-center" required>
                </div>
                <!-- BTN SAVE -->
                <div class="d-flex">
    ...
    <script>
        ...
        createApp({
            ...
            created() {
                this.sketch = JSON.parse(this.sketch);
                this.sketch.forEach(figure => {
                    if(figure.name==='line') {
                        this.figures.push(new Figura(figure.name, figure.x1, figure.y1, figure.x2, figure.y2));
                    } else if(figure.name==='text') {
                        this.figures.push(new Figura(figure.name, figure.x, figure.y, 0, 0));
                        this.figures[this.figures.length-1].updateText(figure.font_size, figure.text);
                    } else {
                        this.figures.push(new Figura(figure.name, figure.x, figure.y, figure.w, figure.h));
                    }
                    this.figures[this.figures.length-1].updateFill(figure.fill_color, figure.fill_opacity);
                    this.figures[this.figures.length-1].updateBorder(figure.stroke_color, figure.stroke_opacity, figure.stroke);
                    this.figures[this.figures.length-1].corner = figure.corner;
                });
            },
            methods: {
                setFig(figura) {
